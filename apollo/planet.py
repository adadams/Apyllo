...
class MakeModelBlueprint(TypedDict):
    ...
    model_pressures: Sequence[float]
    ...
    gas_end_index: int
    TP_model_type: str
    TP_model_function: Callable[[Sequence[float]], Sequence[float]]
    ...
    cloud_model_index: int
    hazetype_index: str
    cloud_parameter_names: Sequence[str]
    cloud_start_index: int
    calibration_parameter_names: Sequence[str]
    calibration_start_index: int
    APOLLO_use_mode: str


# emission_flux:                               Sequence[float]
# radius_parameter:                            float
# radius_case:                                 str
# wavelength_offset:                           float
# model_parameters:                            Sequence[float]
# planet_parameters:                           Sequence[float]
# free_variable_indices:                       Sequence[int]
# basic_parameter_names:                       Sequence[str]
# basic_start_index:                           int
# end_parameter_names:                         Sequence[str]
# end_start_index:                             int
...
class ObserveModelBlueprint(TypedDict):
    ...
    distance_to_system: float
    bin_low_wavelength_indices_by_opacity_bins: Sequence[float]
    bin_high_wavelength_indices_by_opacity_bins: Sequence[float]
    bin_low_wavelength_offset_by_index: Sequence[float]
    bin_high_wavelength_offset_by_index: Sequence[float]
    binned_data_flux: Sequence[float]
    data_downbinning_factor: float
    data_convolution_factor: float
    data_band_bounding_indices: Sequence[Sequence[int]]
    observation_mode_index: int
    normalize_spectrum: bool
    use_default_radius: bool
    data_errors_binned_to_model: Sequence[float]
...
class Planet:
    ...
    def MakeModel(self, model_constructor: MakeModelBlueprint):
        def ModelFunction(
            ...
            if gray:
                tplong = TP_model.evaluate_model(
                    tgray, num_layers_final=vres, P_min=minP, P_max=maxP
                )
                ...
                if cloud_fraction == 1:
                    ...
                    specflux = cloud_fraction * np.asarray(
                        self.cclass.get_Spectrum()
                    ) + (1 - cloud_fraction) * np.asarray(
                        self.cclass.get_ClearSpectrum()
                    )

            # Build atmosphere temperature profile and compute spectrum
            else:
                for i in range(0, len(tpprof)):
                    tpprof[i] = x[i + a1]
                if atmtype == "Parametric":
                    tpprof[1] = 10 ** tpprof[1]
                if atmtype in TP_models:
                    tplong = TP_model.evaluate_model(
                        *tpprof, num_layers_final=vres, P_min=minP - 6, P_max=maxP - 6
                    )
                    # Compute spectrum
                    self.cclass.set_Params(params1, abund, rxsec, tplong)
                    if cloud_fraction == 1:
                ...
                if atmtype == "Layers":
                    if verbatim:
                        ...
                        if natm <= 4 or natm >= 13:
                            f = interp1d(profin, tpprof, kind="linear")
                        else:
                            f = interp1d(profin, tpprof, kind="cubic")
                        for i in range(0, vres):
            ...
            if atmtype == "Piette":
                ...
                if not np.all(monotonic):
                    print(
                        "Profile isn't purely monotonic. The array: {}".format(x[a1:a2])
                    )
            ...
            specflux = np.where(
                np.logical_and(
                    J_boundaries[0] <= wavelengths, wavelengths <= J_boundaries[1]
                ),
                specflux * scaleJ,
                specflux,
            )
            specflux = np.where(
                np.logical_and(
                    H_boundaries[0] <= wavelengths, wavelengths <= H_boundaries[1]
                ),
                specflux * scaleH,
                specflux,
            )
            specflux = np.where(
                np.logical_and(
                    K_boundaries[0] <= wavelengths, wavelengths <= K_boundaries[1]
                ),
                specflux * scaleK,
                specflux,
            )
            specflux = np.where(
                np.logical_and(
                    G395_boundaries[0] <= wavelengths, wavelengths <= G395_boundaries[1]
                ),
                specflux * scaleG395,
                specflux,
            )
            specflux = np.where(
                np.logical_and(
                    G395_ch1_boundaries[0] <= wavelengths,
                    wavelengths <= G395_ch1_boundaries[1],
                ),
                specflux * scaleG395_ch1,
                specflux,
            )
            specflux = np.where(
                np.logical_and(
                    G395_ch2_boundaries[0] <= wavelengths,
                    wavelengths <= G395_ch2_boundaries[1],
                ),
                specflux * scaleG395_ch2,
                specflux,
            )
