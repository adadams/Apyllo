...
Planet::Planet(vector<int> switches, vector<double> waves, vector<double> waveslo, vector<int> mollist, string opacname, string hir, string lor)
{
  ...
  if (!dimin)
    cout << "Opacity Files Not Found" << std::endl;
...
// Overloaded function in case you want pressure-dependent abundances.
// Reads in the variables that define a particular model and sets the parameters
// void Planet::setParams(vector<double> plparams, vector<vector<double> > abund, vector<double> rxsecs, vector<double> tpprofile)
void Planet::setParams(
    vector<double> plparams,
    vector<double> abund,
    vector<double> rxsecs,
    vector<double> tpprofile)
{
...
double Planet::getTeff()
{
  ...
  if (streams == 2)
    tdepthlo = getFlux(wavenslo, "lores", taulayerlo, w0lo, asymlo);
  if (streams == 1)
    tdepthlo = getFluxOneStream(wavenslo, "lores", taulayerlo, tauproflo);
  double totflux = 0.;
...
vector<double> Planet::getSpectrum()
{
  ...
  if (mode <= 1 && streams == 2)
    tdepth = getFlux(wavens, "hires", taulayer, w0, asym);
  else if (mode <= 1 && streams == 1)
    tdepth = getFluxOneStream(wavens, "hires", taulayer, tauprof);
  else if (mode == 2)
    tdepth = transFlux(rs, wavens, "hires");
  else
    printf("Error: invalid settings in getSpectrum.");
  return tdepth;
}
...
vector<double> Planet::getClearSpectrum()
{
  vector<double> tdepth;
  if (mode <= 1 && streams == 2)
    tdepth = getFlux(wavens, "hires", gastaulayer, gasw0, gasasym);
  else if (mode <= 1 && streams == 1)
    tdepth = getFluxOneStream(wavens, "hires", gastaulayer, gastauprof);
  else if (mode == 2)
    tdepth = transFlux(rs, wavens, "hires");
  else
    printf("Error: invalid settings in getSpectrum.");
    ...
    else
    {
      for (int i = 0; i < nspec; i++)
      {
          ...
          for (int j = 0; j < npress; j++)
          {
            for (int k = 0; k < ntemp; k++)
            {
              for (int l = 0; l < nwave; l++)
              {
                ...
                val = bf + ff;
                if (isnan(val) || isinf(val) || val < 1.e-50)
                  val = 1.e-50;
                if (x < ntable)
                  mastertable[j][k][x][i] += val / (double)degrade;
              }
            }
          }
        ...
        else
        {
          ...
          if (!opacin)
            cout << "Opacity File Not Found" << std::endl;
          double temp;
...
// void Planet::setWave(int npoints, vector<double> rxsecs, vector<double> wavelist, vector<vector<double> > abund)
void Planet::setWave(int npoints, vector<double> rxsecs, vector<double> wavelist, vector<double> abund)
{
  ...
  if (mode <= 1)
    getTauProf(wavens, "hires");
  if (mode == 2)
    transTauProf(wavens, "hires");
...
calculating the contribution functions.
    ...
    for (int n3 = 0; n3 < nlayershort; n3++)
    {
      ...
      if (xk2[n3] != 0. && fabs(xk2[n3] / xk[2 * n3] < 1.e-30))
        xk2[n3] = 0.;
    }
...
vector<double> Planet::getFluxOneStream(vector<double> wavens, string table, vector<vector<double>> taulayer, vector<vector<double>> tauprof)
{
  // 8 Gauss points
  ...
  for (int j = 0; j < 8; j++)
  {
    fracflux = getFluxes(wavens, gaussPoints[j], gaussWeights[j], table, taulayer, tauprof);
    for (int i = 0; i < wavens.size(); i++)
    {
...
vector<double> Planet::getFluxes(vector<double> wavens, double cosmu, double delMu, string table, vector<vector<double>> taulayer, vector<vector<double>> tauprof)
{
  if (table == "hires")
  {
    ...
    for (int i = 0; i < wavens.size(); i++)
    {
      ...
      for (int j = nlayer - 1; j >= 0; j--)
      {
        ...
        else if (hprof[j + 1] >= hmin)
        {
          ...
          if (j > 0)
            dflux *= exp(-tauprof[i][j - 1] / cosmu); // absorption above layer
        ...
        else
        {
          ...
          double dflux = I0;
          if (j > 0)
            dflux *= exp(-(tauadjust + tauprof[i][j - 1]) / cosmu);
          else
            dflux *= exp(-tauadjust / cosmu);

          double I1 = blackbodyL(tmid, wavelength);
          double dflux1 = I1 * (1. - exp(-tauadjust / cosmu)); // source function
          if (j > 0)
            dflux1 *= exp(-tauprof[i][j - 1] / cosmu); // absorption above layer
          dflux += dflux1;
          dflux *= sinmu; // multiply by area
          fluxes[i] += dflux;
        }
    ...
    for (int i = 0; i < wavenslo.size(); i++)
    {
      ...
      for (int j = nlayer - 1; j >= 0; j--)
      {
        ...
        else if (hprof[j + 1] >= hmin)
        {
          ...
          if (j > 0)
            dflux *= exp(-tauproflo[i][j - 1] / cosmu); // absorption above layer
          dflux *= sinmu;                               // multiply by area
          fluxes[i] += dflux;
        }
        ...
        else
        {
          ...
          double dflux = I0;
          if (j > 0)
            dflux *= exp(-(tauadjust + tauproflo[i][j - 1]) / cosmu);
          else
            dflux *= exp(-tauadjust / cosmu);
          ...
          double dflux1 = I1 * (1. - exp(-tauadjust / cosmu)); // source function
          if (j > 0)
            dflux1 *= exp(-tauproflo[i][j - 1] / cosmu); // absorption above layer
          dflux += dflux1;
...
void Planet::getProfLayer(vector<double> tpprofile)
{
  ...
  for (int i = nlevel - 2; i >= 0; i--)
  {
    ...
    else
    {
      ...
      if (hprof[i] >= rp * 5.0 || isinf(hprof[i]))
        hprof[i] = rp * 5.0;
    }
  }
}
  ...
  if (table == "hires")
  {
    ...
    for (int i = 0; i < wavens.size(); i++)
    {
      ...
      for (int j = 0; j < nlayer; j++)
      {
        ...
        if (cloudmod > 0 && cloudmod != 5)
        {
          double cloudopacityinlayer = hazedepth * dlc;
          cloudtaulayer[i][j] = cloudopacityinlayer;
          taulayer[i][j] += cloudopacityinlayer;
          ...
          {
            cloudtauprof[i][j] = cloudtauprof[i][j - 1] + cloudopacityinlayer;
            tauprof[i][j] += cloudopacityinlayer;
          }
        }
        ...
        {
          w0[i][j] = sca * nden / (dtau + hazedepth);
        }
    ...
    for (int i = 0; i < wavenslo.size(); i++)
    {
      ...
      for (int j = 0; j < nlayer; j++)
      {
        ...
        else if (cloudmod == 4)
        {
          ...
          w0lo[i][j] = sca * nden / (dtau + hazedepth);
        // w0 is layer single scattering albedo with layer 0 on top
        // ratio of scattering to total opacity
      } // end for(j)
    } // end for(i)
...
// void Planet::getOpacProf(vector<double> rxsecs, vector<double> wavelist, vector<vector<double> > abund, string table){
void Planet::getOpacProf(vector<double> rxsecs, vector<double> wavelist, vector<double> abund, string table)
{
  ...
  if (table == "lores")
    opacproflo = vector<vector<double>>(wavenslo.size(), vector<double>(nlayer, 0));
  ...
  for (int m = 0; m < wavelist.size(); m++)
  {
    ...
    for (int j = 0; j < nlayer; j++)
    {
      ...
      if (table == "hires")
      {
        ...
        {
          // xsec[0] = mastertable[jp][jt][jw][iii]*abund[iii][j];
          // xsec[1] = mastertable[jp][jt+1][jw][iii]*abund[iii][j];
          // xsec[2] = mastertable[jp+1][jt][jw][iii]*abund[iii][j];
          // xsec[3] = mastertable[jp+1][jt+1][jw][iii]*abund[iii][j];
      ...
      if (table == "lores")
      {
        for (int iii = 0; iii < nmol; iii++)
        {
          // xsec[0] += lotable[jp][jt][jw][iii]*abund[iii][j];
          // xsec[1] += lotable[jp][jt+1][jw][iii]*abund[iii][j];
          // xsec[2] += lotable[jp+1][jt][jw][iii]*abund[iii][j];
          // xsec[3] += lotable[jp+1][jt+1][jw][iii]*abund[iii][j];
...
void Planet::getSca(vector<double> rxsecs, vector<double> wavelist, string table)
{
  ...
  if (table == "lores")
    scatablelo = vector<vector<double>>(wavenslo.size(), vector<double>(nlayer, 0));
  ...
  for (int n = 0; n < nmol; n++)
  {
    for (int m = 0; m < wavelist.size(); m++)
    {
      for (int j = 0; j < nlayer; j++)
      {
        ...
        if (table == "lores")
          scatablelo[m][j] += rxsecs[n] * pow(nu / 5.0872638e14, 4.0);
      }
    }
  }
}
